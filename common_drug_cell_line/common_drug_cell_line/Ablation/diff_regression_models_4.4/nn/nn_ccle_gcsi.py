...
Original file is located at
    https://colab.research.google.com/drive/1LgkmzZMKRwvRYLY_Shn1PVE1v5kIYCmW
"""
...
CCLE_main=pd.read_csv(data_path+'CCLE_paired_common_with_gcsi.csv', index_col=0)
gCSI_main=pd.read_csv(data_path+'gCSI_paired_common_with_ccle.csv', index_col=0)

ccle_x = CCLE_main.iloc[:, 0:-1]
ccle_y = CCLE_main.iloc[:, -1]
...
gCSI_main=pd.concat([gCSI_train, gCSI_test], axis=0)
CCLE_main=pd.concat([ccle_train, ccle_test],axis=0)
...
class Net(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        nn.Linear(len(CCLE_main.columns)-1, 64),
        nn.ReLU(),
        nn.Linear(64, 32),
        nn.ReLU(),
        nn.Linear(32, 1)
...
i=0
for train_index, test_index in kf.split(CCLE_main,gCSI_main):

    m1 = Net().to(DEVICE)
    m2= Net().to(DEVICE)

    ccle_trainloader, ccle_testloader = load_data(CCLE_main, train_index, test_index)
...
with open("results/ccle_gcsi_nn.txt", "a") as output:
    output.write("ccle_gcsi_our_method_mae_nn="+str(maes_our)+"\n")
    output.write("ccle_gcsi_our_method_mse_nn="+str(mses_our)+"\n")
    output.write("ccle_gcsi_our_method_r2_nn="+str(r2s_our)+"\n")
