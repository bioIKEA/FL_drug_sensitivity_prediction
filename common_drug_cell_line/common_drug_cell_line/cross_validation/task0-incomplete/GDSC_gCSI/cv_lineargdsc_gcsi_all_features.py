...
for train_index, test_index in kf.split(gdsc_x,gdsc_y):
    ...
    #for m4, gcsi is the x and gdsc is the y, reverse for m3
    m4.fit(gcsi_y_train_pred.reshape(-1, 1), gdsc_y_train_pred)

    # Make predictions using the testing set
    gdsc_y_pred_m4 = m4.predict(gcsi_y_pred.reshape(-1,1))

    #avg gdsc values predicted by m4 + gcsi input y values  to m4 
    gcsi = (gdsc_y_pred_m4 + gcsi_y_pred)/2

    #print(gdsc_y_train_pred.reshape(-1, 1))
    #print(gdsc_y_train_pred)

    m3 =MLPRegressor(random_state=1, max_iter=200)
    #for m3, gcsi is the y and gdsc is the x, reverse for m4
    m3.fit(gdsc_y_train_pred.reshape(-1, 1), gcsi_y_train_pred)

    # Make predictions using the testing set
    gcsi_y_pred_m3 = m3.predict(gdsc_y_pred.reshape(-1,1))

    gdsc = (gcsi_y_pred_m3 + gdsc_y_pred)/2
...
for train_index, test_index in kf.split(gdsc_x,gdsc_y):
    ...
    #stack the avg_y_pred twice for GDSC and gCSI. first half of mixed_y_test comes from GDSC and the second half comes from gCSI
